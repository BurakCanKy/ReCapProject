...
namespace ConsoleUI
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            customerManager.Add(new Customer { UserId = (userManager.GetById(user.UserId).Data.UserId), CustomerName = " Güçü var  Şirket" });
            ...
            foreach (var results in result2.Data)
            {
                Console.WriteLine("{0}|{1}|{2}|{3}|{4}",
                    results.CarId, results.CarName, results.UserName, results.CustomerName, results.RentDate, results.ReturnDate);
            }
